...
data:any;
  ...
  onAccountTypeChange() {
    if (this.formData.corporateCode && this.formData.accountType) {  
      this.corporateCodeId = this.filteredUsers.filter(item => item.corporateCode === this.formData.corporateCode)[0].id;
       const url =
        baseUrl +
  ...
  fetchData(id: number): void {
    ...
    this.http.get<any>(`http://167.172.220.75:8084/CashflowForecastingApplication/api/forecasts/${id}`)
      .subscribe({
        next: (response) => {
  ...
  updateData(id: number, data: any): void {
    this.http.put<any>(`http://167.172.220.75:8084/CashflowForecastingApplication/api/forecasts/${id}`, data)
      .subscribe({
        next: (response) => {
          if (response.code === 200 && response.status === 'success') {
